...
title: Inseason analysis of chum salmon (*Oncorhynchus* *keta*) bycatch from the shoreside
  ...
  trawl fishery
author: "Patrick Barry, Jamie Musbach, Abby Duffy, Jason Smith, Jordan Head, and Wes Larson"
...
# Summary
```{r exec-summary, echo = FALSE, ref.label = c("readAKRO", "AKROsummary","BBSRIsampling","GTdat","PositiveControl","QC_input","NameFunkiness","SampGeno","SetupRuns", "BSmodelClean")}
```
...
```{r SumTab, echo = FALSE, message=FALSE,out.width='4in',fig.align='center'}
...
#If we are combining weeks do so now.
if(CmbWk==TRUE){
  AKRO_Obs <- AKRO_Obs %>%
    mutate(SW = ifelse(SW %in% SW2Cmb, CrntSW,SW))
}
...
```{r Map, eval=T, echo=F, fig.cap='Map of National Marine Fisheries Service management areas within the Bering Sea and Aleutian Islands. Colored areas indicate management areas where chum salmon bycatch has historically occurred.',fig.width=4}
...
for processing. Genomic DNA was extracted from dried fin clips 
...
individuals are identified to have matching multilocus genotypes (>95% similarity)
the individuals with fewer scored loci was dropped. Genetic stock identification 
was performed with the conditional 
genetic stock identification model in the R package *rubias*
[@MoranAnderson2019] following the methods used in NOAA's annual bycatch reports.
...
run, with each chain starting at disparate values of stock 
...
```{r PositiveControl, eval=T, echo=F}
#### compare positive controls with known genotypes


#Remove the positive controls
Genos_Fldm_Alleles <- Genos_Fldm_Alleles %>%
  filter(!(indiv %in% PosCntrls))
```
...
#Too many issues with AKRO data right now, this is the old code that merged the data
...
#   group_by(SW.x,TARGET_SAMPLE_RATE) %>%#group_by(PLANT_NAME.y,SW.x,TARGET_SAMPLE_RATE) %>%
...
ProcessRate <- GenoMetaDat %>%
  left_join(.,BBSRI_Dlvry,by="TEMP_DELIVERY_ID")%>% #
  filter(SEL_TO_GENOTYPE == 1)%>%
  mutate(SuccssGTd = ifelse(`WHAT_CARD-NO`%in%Mixtures$indiv,1,0))%>%
  rename('SW' = 'SW.x')%>%
  mutate(SW = ifelse(CmbWk==TRUE & SW %in% SW2Cmb, CrntSW,SW))%>%
...
AKRO_BBSRI_DlvrySum <- BBSRI_Dlvry %>%
  group_by(PLANT_NAME,SW,TARGET_SAMPLE_RATE) %>%
  mutate(TARGET_SAMPLE_RATE = round(1/TARGET_SAMPLE_RATE,3))%>%
  summarize(nChum = sum(CHUM_LANDED,na.rm=T),
            nChumSamp = sum(CHUM_SAMPLED,na.rm=T),
            RlzdSampRt = round(nChumSamp / nChum,3))%>%
  left_join(ProcessRate%>% mutate(SW=as.character(SW)) )%>%
  mutate(GenoRate = round(nGenod/nChum,3))%>%
bind_rows(.,data.frame(PLANT_NAME = "Total",
                       nChum = sum(BBSRI_Dlvry$CHUM_LANDED,na.rm=T),
                       nChumSamp = sum(BBSRI_Dlvry$CHUM_SAMPLED,na.rm=T),
                       RlzdSampRt = round(sum(BBSRI_Dlvry$CHUM_SAMPLED,na.rm=T)/sum(BBSRI_Dlvry$CHUM_LANDED,na.rm=T),2),
                       nPrcsd = sum(ProcessRate$nPrcsd,na.rm=T),
                       nGenod = sum(ProcessRate$nGenod,na.rm=T),
                       GenoRate = round(sum(ProcessRate$nGenod,na.rm=T)/sum(AKRO_BBSRI_Dlvry$CHUM_LANDED,na.rm=T),3)
                       ))%>%
  filter(nChum>0)%>%ungroup()%>%
  mutate(PLANT = recode(PLANT_NAME,
                             'Alyeska' = 'P1',
                             'Northern Victor' = 'P2',
                             'Trident' = 'P3',
                             'Unisea' = 'P4',
                             'Westward' = 'P5'))%>%
  select(-PLANT_NAME) %>%relocate(PLANT,.before='SW')
...
AKRO_BBSRI_DlvrySum %>%
 ...
 kableExtra::row_spec(nrow(BBSRI_Dlvry %>%mutate(PLANT = recode(PLANT_NAME,
                             'Alyeska' = 'P1',
                             'Northern Victor' = 'P2',
                             'Trident' = 'P3',
                             'Unisea' = 'P4',
                             'Westward' = 'P5'))%>%
  select(-PLANT_NAME) %>%
                  group_by(PLANT,SW,TARGET_SAMPLE_RATE) %>%
                  mutate(TARGET_SAMPLE_RATE = round(1/TARGET_SAMPLE_RATE,3))%>%
                  summarize(nChum = sum(CHUM_LANDED,na.rm=T),
                            nChumSamp = sum(CHUM_SAMPLED,na.rm=T),
...
AKRO_BBSRI_DlvrySum %>%
  ...
  rename('nGTd' = 'nPrcsd' , 
         'nAnalyzed' = 'nGenod' )%>%
  select(-c(TARGET_SAMPLE_RATE,GenoRate,RlzdSampRt))%>%
write.csv(x = .,
            file = file.path(paste("./Output/",CrntYr,"/",CrntSW,"/SW",CrntSW,"_SamplingDetails.csv",sep="")),
            row.names=F,
            quote =F)
```
...
```{r PriorResChk, eval=T, echo=F, cache=T}
...
```{r PriorResGraph, eval=PriorRes, echo=F, fig.height=6,fig.width=7, fig.cap="Chum salmon bycatch proportion (A) and total number of fish (B) for the B season of the pollock trawl fishery."}
GsiRes2Read <- list.files(file.path(paste("./Output/",CrntYr,"/",sep="")),recursive=T,full.names = T)%>%
  grep(pattern="GSIresults_",value=T)
