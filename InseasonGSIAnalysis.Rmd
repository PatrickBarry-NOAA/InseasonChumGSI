...
# Summary
```{r exec-summary, echo = FALSE, ref.label = c("readAKRO", "AKROsummary","BBSRIsampling","GTdat","QC_input","NameFunkiness","SetupRuns","BSmodelClean")}
```

There were `r AKRO_ObsSum %>% filter(NMFS_AREA=="BSAI Total")%>% filter(SW==CrntSW)%>%ungroup() %>% select(nChum) %>% sum() %>% formatC(.,big.mark = ",")` chum salmon caught in 
...
`r (AKRO_BBSRI_Dlvry$CHUM_SAMPLED %>% sum(.,na.rm=T) / AKRO_ObsSum %>% filter(NMFS_AREA=="BSAI Total")%>% filter(SW==CrntSW)%>%ungroup() %>% select(nChum) %>% sum()) %>% round(.,2)`. Of those 
that were sampled, `r paste((GenoMetaDat%>%nrow() -nrow(BadInds) - nDups) ," (",((GenoMetaDat%>%nrow() -nrow(BadInds) - nDups)/AKRO_BBSRI_Dlvry %>% filter(SW == CrntSW) %>% select(CHUM_SAMPLED) %>% sum(.,na.rm=T))*100 %>% round(.,3),"%)",sep="")` passed quality control filters and were analyzed. 
...
-Add fishery info about sectors
-Add management as non-chinook but mostly chum
...
Allowed access to chum salmon bycatch after NMFS observers 
have processed the offload, BBSRI technicians take a length measurement, scale sample for 
...
delivery. Scale samples are mounted on gum cards and stored for post-season analysis.
...
the indviduals with fewer scored loci is dropped. Genetic stock identification 
...
write.csv(file = file.path(paste("./Output/",CrntYr,"/",CrntSW,"/GSIresults_SW",CrntSW,".csv",sep="")),
          x= Res_temp,
          row.names=F,
          quote=F)
```
...
In statistical week `r CrntSW`, `r Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% slice_max(Mean) %>% select(Region) %>% unlist()` comprised the largest proportion of the chum salmon bycatch (`r Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% slice_max(Mean) %>% select(Mean) %>% unlist() %>% {.*100} %>% round(.,1)`%). Of the 
...
`r Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% slice_max(EstNum) %>% select(EstNum) %>% unlist() %>% round(.,0)`
were from `r Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% slice_max(Mean) %>% select(Region) %>% unlist()`.
...
fish. `r if(any(Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% select(EstNum) %>% round(.,0) %>% {.==0})){paste("No chum salmon were caught from the ", Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% mutate(EstNum = round(EstNum,0)) %>% filter(EstNum == 0) %>% select(Region) %>% unlist() %>% paste(.,collapse=", ") %>% {sub(",([^,]*)$", " &\\1", .)},".",sep="")}`
...
if(length(GsiRes2Read)>1){
  GSI_ResSWs <- lapply(1:length(GsiRes2Read),function(x) read.csv(GsiRes2Read[x])) %>%
                         do.call(rbind,.)
}
