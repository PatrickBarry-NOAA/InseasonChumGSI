...
title: Inseason analysis of chum salmon (*Oncorhynchus* *keta*) bycatch from shoreside
  sector of Bering Sea Aleutian Islands walleye pollock (*Gadus* *chalcogrammus*)
  trawl fishery.
...
CrntSW <- 33 #Current Stat week
CrntYr <- format(Sys.Date(),"%Y") #Get the current year
RunRubias <- FALSE #On/Off for running the GSI analysis
...
# Summary
```{r exec-summary, echo = FALSE, ref.label = c("readAKRO", "AKROsummary","BBSRIsampling","GTdat","QC_input","NameFunkiness","SampGeno","SetupRuns","CondModel","BootstrapModels", "BSmodelClean")}
```

In statistical week `r CrntSW` of the Bering Sea Aleutian Islands (BSAI)
walleye pollock trawl fishery there were
`r AKRO_ObsSum %>% filter(NMFS_AREA=="BSAI Total")%>% filter(SW==CrntSW)%>%ungroup() %>% select(nChum) %>% sum() %>% formatC(.,big.mark = ",")` chum salmon caught by the shoreside sector. 
Of that total catch, `r AKRO_BBSRI_Dlvry$CHUM_SAMPLED %>% sum(.,na.rm=T)`
were sampled, `r ProcessRate$nPrcsd %>% sum(.,na.rm=T)` were selected for genotyping, 
and `r ProcessRate$nGenod %>% sum(.,na.rm=T)` (`r (ProcessRate$nGenod %>% sum(.,na.rm=T) / AKRO_ObsSum %>% filter(NMFS_AREA=="BSAI Total")%>% filter(SW==CrntSW)%>%ungroup() %>% select(nChum) %>% sum()) %>% {.*100} %>% round(.,1)`%)  were successfully genotyped to determine the 
genetic stock composition of the bycatch. `r ifelse(AKRO_BBSRI_DlvrySum$GenoRate[-length(AKRO_BBSRI_DlvrySum)] %>% {.[!is.na(.)]} %>% dist(.) %>% {any(.>0.1)}, "Genotyping rates varied substantially (>10%) across processing plants suggesting that the collection of samples may not be representative of the overall bycatch. Samples were analyzed with the understanding that some bias may exist in the spatial and temporal coverage of the samples.", "")` `r Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% slice_max(Mean) %>% select(Region) %>% unlist()` comprised the largest proportion of the chum salmon bycatch (`r Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% slice_max(Mean) %>% select(Mean) %>% unlist() %>% {.*100} %>% round(.,1)`%), 
...
Islands (BSAI), chum salmon (*Oncorhynchus* *keta*) incidental catch occurs occasionally in 
large numbers. Salmon are managed as a prohibited species catch (referred to as bycatch) 
and are highly regulated. The fishery is composed of three distinct processing sectors, each 
with distinct operational constraints. Within the shoreside sector, smaller vessels
which lack the ability to process hauls at-sea often fish closer to the Alaksa Peninsula. 
Differences in the stock specific distribution of chum salmon within the Bering Sea
result in the shoreside sector often have the largert proportion and number of Western Alaska (Coastal Western Alaska, Yukon Fall run, and 
Kotzebue Sound genetic groups combined) chum salmon bycatch relative to the other sectors.
Currently, 
annual estimates of genetic stock composition are produced by
...
Salmon Research Institute (BBSRI) to sample the bycatch from the shoreside 
...
```{r Map, eval=T, echo=F, fig.cap='Map of NMFS areas within the Bering Sea and Aleutian Islands from chum salmon bycatch has historically been caught.'}
...
ggplot() + 
  ...
  ggsn::scalebar(data=nmfs,
                 ...
                 height = 0.005, st.dist = 0.0075, st.bottom = TRUE, st.size = 3,
                 anchor=c(x=360-162,y=53.5))+
   geom_text(data=StatAreaLabs,aes(x=long,y=lat,label=Area),size=3)+
...
delivery. Scale samples were mounted on gum cards and stored for post-season analysis.
...
technicians sampled hauls delivered to Dutch Harbor and Akutan. A total of `r AKRO_BBSRI_Dlvry$CHUM_SAMPLED %>% sum(.,na.rm=T)` chum salmon
...
~`r as.character((AKRO_BBSRI_Dlvry$CHUM_SAMPLED %>% sum(.,na.rm=T) / AKRO_ObsSum %>% filter(NMFS_AREA=="BSAI Total")%>% filter(SW==CrntSW)%>%ungroup() %>% select(nChum) %>% sum()) %>% janitor::round_to_fraction(.,denominator = 30) %>% MASS::fractions(.))`. `r ifelse(AKRO_BBSRI_Dlvry %>% filter(is.na(TARGET_SAMPLE_RATE) & CHUM_RETENTION_COUNT > 0) %>% select(CHUM_RETENTION_COUNT) %>% sum(.,na.rm=T) %>% {.>0},paste("There were ", AKRO_BBSRI_Dlvry %>% filter(is.na(TARGET_SAMPLE_RATE) & CHUM_RETENTION_COUNT > 0) %>% select(CHUM_RETENTION_COUNT) %>% sum(.,na.rm=T), " chum salmon unsampled from ", AKRO_BBSRI_Dlvry %>% filter(is.na(TARGET_SAMPLE_RATE)  & CHUM_RETENTION_COUNT > 0) %>% nrow(.), " deliveries",sep=""),"All deliveries with non-zero chum bycatch were sampled")` (Table 1). `r ifelse(any(0 %in% GenoMetaDat$SEL_TO_GENOTYPE), paste("Not all chum salmon sampled were genotyped. Of the ", AKRO_BBSRI_Dlvry$CHUM_SAMPLED %>% sum(.,na.rm=T), " samples collected, " , ProcessRate$nPrcsd %>% sum(.,na.rm=T), " were selected for genotyping with ",  ProcessRate$nGenod %>% sum(.,na.rm=T)," (",(ProcessRate$nGenod %>% sum(.,na.rm=T) / AKRO_ObsSum %>% filter(NMFS_AREA=="BSAI Total")%>% filter(SW==CrntSW)%>%ungroup() %>% select(nChum) %>% sum()) %>% {.*100} %>% round(.,1),"%) successfully genotyped.",sep=""),paste("All chum salmon sampled were processed for genotyping with ", ProcessRate$nGenod %>% sum(.,na.rm=T)," (",(ProcessRate$nGenod %>% sum(.,na.rm=T) / AKRO_ObsSum %>% filter(NMFS_AREA=="BSAI Total")%>% filter(SW==CrntSW)%>%ungroup() %>% select(nChum) %>% sum()) %>% {.*100} %>% round(.,1),"%) successfully genotyped.",sep="")) `




<!-- which  -->
<!-- is `r ifelse((AKRO_BBSRI_Dlvry$CHUM_SAMPLED %>% sum(.,na.rm=T) / AKRO_ObsSum %>% filter(NMFS_AREA=="BSAI Total")%>% filter(SW==CrntSW)%>%ungroup() %>% select(nChum) %>% sum())  < (1/min(as.numeric(AKRO_BBSRI_Dlvry$TARGET_SAMPLE_RATE),na.rm=T)), "less than", "greater than")` the target rate of `r as.character(1/min(as.numeric(AKRO_BBSRI_Dlvry$TARGET_SAMPLE_RATE),na.rm=T) %>% janitor::round_to_fraction(.,denominator = 30) %>% MASS::fractions(.))` (`r (1/min(as.numeric(AKRO_BBSRI_Dlvry$TARGET_SAMPLE_RATE),na.rm=T) %>% janitor::round_to_fraction(.,denominator = 30) %>% MASS::fractions(.))`; Table 1). -->
...
```{r ChangeAlleles, eval=F, echo=F}
#Deal with BBSRI allele swap issues

# Genos_Fldm_AllelesDummy <- Genos_Fldm_Alleles
# 
# AlleleList<-list()
# 
# for (i in 5:ncol(Genos_Fldm_AllelesDummy)){
#   Alleles <- Genos_Fldm_AllelesDummy %>%
#     select(all_of(i)) %>%
#     table() %>%
#     names() %>% unlist()
#   
#   AlleleSubDF = data.frame(Old = Alleles,
#                            New = rev(Alleles))
#   
#   AlleleList[[i-4]]<- Genos_Fldm_AllelesDummy %>%
#     select(all_of(i))%>%
#     unlist() %>%
#     {plyr::mapvalues(from = AlleleSubDF$Old,
#                     to = AlleleSubDF$New,
#                     x = .)}%>%
#     as.vector()
#   
# }
# 
# Genos_Fldm_AllelesDummy<-bind_cols(Genos_Fldm_AllelesDummy[,1:4],do.call(cbind,AlleleList))
# 
# colnames(Genos_Fldm_AllelesDummy)<- colnames(Genos_Fldm_Alleles)
...
AKRO_BBSRI_DlvrySum %>%
  select(-SW) %>%
  knitr::kable(col.names=c("Plant","Target Sample\nRate","Total\nChum","Chum\nSampled","Sample\nRate","Chum\nGenotyped","Chum\nAnalyzed","Successful Genotype\nRate"), booktabs = T, caption = paste("Sampling and genotyping information for statistical week " , CrntSW, " for the shoreside processing plants.",sep=""))%>%
 kableExtra::row_spec(nrow(AKRO_BBSRI_Dlvry %>%
  ...
  kableExtra::column_spec(2:8, width = c("2cm","1cm","2cm","1cm","2cm","1cm","1cm"))
```
...
```{r CondModel, eval=RunRubias, echo=F}
...
```{r BootstrapModels, eval=RunRubias, echo=F}
...
Res_temp<-FullRes %>%
   ...
   mutate(EstNum = PSCtemp*Mean,
          EstLo = PSCtemp*`2.5% CI`,
          EstHi = PSCtemp*`97.5% CI`)%>%
...
TabTitleNumb<-paste("Table 2: ",gsub(pattern="Stat","Statistical",Labtemp)," (PSC = ",formatC(PSCtemp,big.mark=",",format="d"),
                "; ",
...
bycatch. `r Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% slice_max(Mean) %>% select(Region) %>% unlist()` comprised the largest proportion of the chum salmon bycatch (`r Res_temp %>% filter(Analysis == paste("SW",CrntSW,sep="")) %>% slice_max(Mean) %>% select(Mean) %>% unlist() %>% {.*100} %>% round(.,1)`%), 
